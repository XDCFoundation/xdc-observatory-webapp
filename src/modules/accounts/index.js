...
export default class LatestAccountsList extends BaseComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      tableColumns: {
        ...
        Balance: {
          isActive: true,
          toolTipText: "Number of token present in a particular address",
        },
      },
    };
  }
  ...
  async getListOfAccounts(from, amount, keywords = "") {
    ...
    let [error, listOfAccounts] = await Utils.parseResponse(
      AccountService.getLatestAccount(urlPath, {})
    );

    if (error || !listOfAccounts) return;
  ...
  async getCoinMarketTotalSupply() {
    let [error, coinMarketTotalSupply] = await Utils.parseResponse(
      CoinMarketService.getCoinMarketTotalSupply()
    );
    if (error || !coinMarketTotalSupply) return;
    this.setState({ totalSupply: coinMarketTotalSupply });
  }

  async getTotalAccounts() {
    let [error, totalNumberAccounts] = await Utils.parseResponse(
      AccountService.getTotalAccount()
    );
    if (error || !totalNumberAccounts) return;
