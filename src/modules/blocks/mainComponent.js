...
export default class LatestBlocksList extends React.PureComponent {
  ...
  async getListOfBlocks(from, amount, sortKey) {
    ...
    let [error, listOfBlocks] = await Utils.parseResponse(
      BlockService.getLatestBlock(urlPath, {})
    );
    if (
      !listOfBlocks ||
      listOfBlocks.length === 0 ||
      listOfBlocks === undefined ||
      listOfBlocks === "" ||
      listOfBlocks === null
    ) {
      this.setState({ isLoading: false });
    }
    if (error || !listOfBlocks) return;
    this.setState({ blocksList: listOfBlocks });
    this.setState({ isLoading: false });
  }

  async getTotalNumberOfBlocks() {
    let [error, totalB] = await Utils.parseResponse(
      BlockService.getTotalBlocks()
    );
    if (error || !totalB) return;
  ...
  _handleChange = (event) => {
    ...
    if (this.state.lastPage === true) {
      this.getListOfBlocks(this.state.lastFrom, event.target.value, 1);
    } else {
      this.getListOfBlocks(this.state.from, event.target.value, -1);
    }
  };
  ...
  _FirstPage = (event) => {
    ...
    this.getListOfBlocks(0, this.state.amount, -1);
  };
  _LastPage = (event) => {
    ...
    let lastFrom = 0;
    this.getListOfBlocks(lastFrom, this.state.amount, 1);
  ...
  _NextPage = async (event) => {
    ...
    if (this.state.lastPage === true) {
      if (this.state.lastFrom - this.state.amount >= 0) {
        ...
        this.getListOfBlocks(from, this.state.amount, 1);
      }
    } else {
      if (+this.state.amount + +this.state.from < this.state.totalblocks) {
        let from = +this.state.amount + +this.state.from;
